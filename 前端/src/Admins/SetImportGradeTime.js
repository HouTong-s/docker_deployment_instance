...
const {Search} = Input;
const {Option} = Select;
class SetImportGradeTime extends React.Component
{
    constructor(props)
    {
        super(props);
    ...
    getinfo()
    {
        ...
        axios.get('/api/admins/GradeRegisterStatus',
        {
    ...
    Modify()
    {
        ...
        var that = this;
        axios.post('/api/admins/GradeRegisterStatus',
        {
            gradeBeginTime:that.state.beginTime,
            gradeEndTime:that.state.endTime,
        ...
        .then(function (response) {
            let data = response.data;
            message.info("设置成绩导入状态成功");
            that.getinfo();
        })
        .catch(function (error) {
            if(isExpire(error))
                that.props.redirect();
            message.error("设置成绩导入状态失败");
        });
    }
        ...
        */
        ...
        return (
            ...
            <div>
                ...
                <Option value={0}> 不开放成绩导入</Option>
                <Option value={1}> 开放成绩导入</Option>
                </Select>
                ...
                <Button onClick={this.Modify.bind(this)}>设置成绩导入状态</Button>
...
export default SetImportGradeTime;